# 全流程自动扫描+自动交易 — 实施方案（更新版 2026-05-18）
...
- `position_tracker.py` — 持仓管理（RLock 保护 + JSON 持久化 + mark/equity exposure + DCA/DeRisk 已实现盈亏记录 + 分档胜率统计）
...
- `run_live_trading.py` — 主入口（日亏损熔断 + 余额同步+暴跌保护 + 50% crash guard + 可用余额开仓上限）
...
flowchart TD
    ...
    SCORE --> BTC_CAP["BTC 1h 趋势后处理<br>WEAKNESS: LONG score≥6 cap→5<br>STRENGTH: SHORT score≥6 cap→5<br>极值异常(RSI<15+vol≥2.0) 绕过"]
```
...
| Phase 1 扫描 | `MarketScanner.scan()` (纯 Python, 7 指标+12 Alpha 因子+评分+排名, <1s) | 代码 | ✅ |
...
class LiveTradingConfig:
    ...
    default_scan_interval_minutes: int = 5  # 扫描间隔
    pair_whitelist: List[str] = []     # 白名单，空=Top-N模式
```

含 3 种预设：`default`、`conservative`（流动性 200 万、R:R 1.5、仓位 2%、冷却60min）、`aggressive`（流动性 50 万、R:R 0.8、仓位 10%、冷却15min）。`validate()` 方法校验阈值合法性。
...
JSON 持久化到 `~/.vibe-trading/positions.json`，权限不足时降级内存运行。`_initial_balance` 不持久化，以构造函数值为准。
...
| NEUTRAL | watch_only=False（高分信号信赖 Phase 1） | watch_only=True |
| FAIL | 跳过该信号 | 跳过该信号 |
...
每 interval 分钟 (--interval 可配, 默认 10):
  ...
  │   │     ├─ Phase 1 scan (7指标+12Alpha因子+双向评分+price_tier+极值cap)
  │   │     ├─ BTC 1h 趋势后处理 (评分 cap/WEAKNESS+STRENGTH)
  │   │     └─ 分级决策 → Phase2Requests
  │
  ├─ 余额同步 (get_account_balance)
  ...
  │   ├─ Execution Gate (5 项)
  │   ├─ Phase 2 override: NEUTRAL+Gate PASS → WATCH_ONLY
...
| `--pairs BTC ETH SOL` | 交易对白名单（空=Top-N 模式, 也支持 TRADING_PAIRS 环境变量） |
...
| `extensions/live_trading/engine/position_tracker.py` | ✅ | 持仓管理 (mark/equity exposure + DeRisk + 分档胜率 + trailing 持久化) |
